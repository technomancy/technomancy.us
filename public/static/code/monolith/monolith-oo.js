...
board.setup = function() { 
    ...
    this.tds = $A($$('td'));
    this.cells = this.tds.map(function(td) { cells.push(new Cell(td)); });
    this.render();
};

board.whoseTurn = player1;
...
board.onmousedown = function(event) {
    ...
    board.selectedCell.className = "selected";
};
...
function Cell(td) {
    ...
    // relationships between DOM object and Cell object
    this.td = td;
    this.td.cell = this;
...
Cell.prototype.render = function () { 
    ...
    if(this.player) this.td.style.backgroundColor = this.player.color
	else {
